# Model providers — comparison and selection notes

This guide is about **picking a provider** for the agent: what each
built-in model costs to try, how it authenticates from a browser,
and what made these providers fit the library (and others not). For the
mechanics of wiring a model — the contract, the events, the
`Agent`/`<AgentShell>` plumbing — see the
[agent integration guide](agent-integration.md).

## At a glance

| Provider / model | Channel | Architecture | Free way to try it | Browser auth |
|---|---|---|---|---|
| **Gemini** — `GeminiLiveModel` | streaming voice | native speech-to-speech | AI Studio free-tier API key (rate-limited; dense surfaces burn it fast — see the [extensions guide](extensions.md) for the cost levers) | ephemeral token, minted server-side (`mintGeminiToken`) |
| **Gemini** — `GeminiTextModel` | text | chat (request/response) | same free-tier key | `baseUrl` key proxy (or raw key, dev only) |
| **Anthropic** — `AnthropicTextModel` | text | chat (request/response) | no free tier — pay-as-you-go console credit | `baseUrl` key proxy (or raw key, dev only) |
| **OpenAI** — `OpenAITextModel` | text | chat (request/response) | no free tier — pay-as-you-go | `baseUrl` key proxy (or raw key, dev only) |
| **OpenAI** — `OpenAIRealtimeModel` | streaming voice | native speech-to-speech | no free tier; metered per audio token (as of mid-2026, `gpt-realtime-2` is ~$32/M audio-in, ~$64/M audio-out) | ephemeral client secret (`mintOpenAIRealtimeSecret`) |
| **Deepgram** — `DeepgramVoiceAgentModel` | streaming voice | managed STT→LLM→TTS pipeline | **$200 signup credits, no card** — the cheapest way to try a voice model (agent runtime ~$4.50/h as of mid-2026) | short-lived grant JWT (`mintDeepgramToken`) via the WebSocket subprotocol |
| **Hume** — `HumeEviModel` | streaming voice | native speech-to-speech (empathic — models prosody/emotion) | **free plan with monthly credits** | OAuth access token (`fetchHumeAccessToken`) as a query parameter |

Default models (all overridable via each constructor's `model` option;
...
- **Best tool-calling voice:** OpenAI Realtime (`gpt-realtime-2`
  ...
  price. It is also the only voice model here with a true
...
The model adapters keep the **whole agent definition client-side** — the
...
4. **Adaptable audio.** The contract fixes mic input at 16 kHz PCM and
   speaker output at 24 kHz PCM; the adapter must be able to bridge
   to that (resampling/unpacking inside it is fine — OpenAI
...
- **ElevenLabs Agents** — polished platform and a free tier, but the
  ...
  inverts this library's design, where `AgentDefinition` is the single
...
provider directory under `src/lib/agent/<provider>/` — see the
