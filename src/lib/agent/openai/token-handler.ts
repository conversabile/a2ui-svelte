export interface MintOpenAIRealtimeSecretOptions {
	...
	apiKey: string;
	/** Realtime model the secret is scoped to. Default `'gpt-realtime-2'` (keep in sync with the transport). */
	model?: string;
	 ...
	 * instructions, turn detection…). The transport still sends its own
 ...
 * `OpenAIRealtimeTransport`. Throws on failure with a normalised Error —
