...
import { DeepgramVoiceAgentModel } from './agent-model';
...
function listen(model: DeepgramVoiceAgentModel) {
	const ev = {
		textOut: [] as string[],
		textIn: [] as string[],
		audioOut: [] as string[],
...
async function connected(model: DeepgramVoiceAgentModel, tools = TOOLS) {
...
describe('DeepgramVoiceAgentModel', () => {
	it('authenticates via subprotocol and sends the full agent definition as Settings', async () => {
		const model = new DeepgramVoiceAgentModel({ token: 'dg-api-key' });
		listen(model);
	...
	it('uses the bearer scheme for JWT-shaped tokens (minted grants)', async () => {
		...
		const model = new DeepgramVoiceAgentModel({ token: async () => jwt });
		listen(model);
		const ws = await connected(model);
	...
	it('rejects connect() when Deepgram refuses the settings', async () => {
		const model = new DeepgramVoiceAgentModel({ token: 'k' });
		const connectPromise = model.connect({ systemInstruction: 'sys', tools: [] });
		const ws = FakeWebSocket.last;
		ws.open();
	...
	it('surfaces client-side function calls and answers with FunctionCallResponse', async () => {
		...
		model.sendToolResult('f1', 'click_button', { status: 'success' });
	...
	it('close() is idempotent and gates sends', async () => {
		...
		model.sendText('hi');
		model.sendAudioChunk(bytesToBase64(new Uint8Array([1, 2])));
